...
raster_sources <- tibble::tribble(
  ~taxon_class, ~sdm_method, ~dir_path,
  "Mammalia",   "PPM",       "mammal_ppm_bin",
  "Mammalia",   "RangeBag",  "mammal_rangebag_bin",
  "Aves",       "PPM",       "bird_ppm_bin",
  "Aves",       "RangeBag",  "bird_rangebag_bin"
)
...
paths <- list(
  ...
  # Presence rasters (all SDM sources)
...
for (i in seq_len(nrow(paths$raster_sources))) {
  need_dir(
    paths$raster_sources$dir_path[[i]],
    paste0(
      paths$raster_sources$dir_path[[i]], " (",
      paths$raster_sources$taxon_class[[i]], ", ",
      paths$raster_sources$sdm_method[[i]], " presence rasters)"
    )
  )
}
...
raster_counts <- paths$raster_sources |>
  dplyr::mutate(n_bin = vapply(dir_path, count_bin, integer(1)))

assert(
  sum(raster_counts$n_bin) > 0,
  "No '*_bin' rasters found across the configured raster source directories."
)

if (isTRUE(HAS_MAMMAL_ROWS)) {
  assert(
    sum(raster_counts$n_bin[raster_counts$taxon_class == "Mammalia"]) > 0,
    "No mammal '*_bin' rasters found across mammal PPM / RangeBag directories."
  )
}

if (isTRUE(HAS_BIRD_ROWS)) {
  assert(
    sum(raster_counts$n_bin[raster_counts$taxon_class == "Aves"]) > 0,
    "No bird '*_bin' rasters found across bird PPM / RangeBag directories."
  )
}
...
list_raster_manifest <- function(raster_sources) {
  out <- lapply(seq_len(nrow(raster_sources)), function(i) {
    ...
    files <- list.files(src$dir_path[[1]], full.names = TRUE)
    if (!length(files)) return(NULL)

    tibble::tibble(
      taxon_class = src$taxon_class[[1]],
      sdm_method  = src$sdm_method[[1]],
      raster_dir  = src$dir_path[[1]],
      raster_path = files,
      raster_file = basename(files),
      raster_stem = tools::file_path_sans_ext(basename(files)) |>
        stringr::str_to_lower()
    ) |>
      dplyr::filter(stringr::str_ends(raster_stem, "_bin")) |>
      dplyr::mutate(
        raster_candidate = stringr::str_replace(raster_stem, "_bin$", ""),
        sdm_priority = dplyr::if_else(sdm_method == "PPM", 0L, 1L)
      )
  })
...
raster_manifest <- list_raster_manifest(paths$raster_sources)
...
# ------------------------------------------------------------------------------
# 2) Resolve ONE raster + ONE trait per row
#    Raster preference:
#      1) PPM over RangeBag
#      2) original name over synonym
#    Trait preference:
#      1) original name over synonym
# ------------------------------------------------------------------------------
cands <- dplyr::bind_rows(
  ...
  dplyr::mutate(
    ...
    name_prio   = dplyr::if_else(kind == "original", 0L, 1L)
  )
...
raster_cands <- cands |>
  dplyr::left_join(
    ...
    by = c("taxon_class", "stem" = "raster_stem")
  ) |>
  dplyr::mutate(raster_ok = !is.na(raster_path))

raster_pick <- raster_cands |>
  dplyr::filter(raster_ok) |>
  dplyr::group_by(row_id) |>
  dplyr::arrange(sdm_priority, name_prio, candidate, .by_group = TRUE) |>
  dplyr::slice(1) |>
  dplyr::ungroup() |>
  dplyr::transmute(
    row_id,
    name_raster  = candidate,
    match_raster = kind,
    raster_stem  = stem,
    sdm_method,
    raster_dir,
...
trait_pick <- cands |>
  dplyr::filter(trait_ok) |>
  dplyr::group_by(row_id) |>
  dplyr::arrange(name_prio, candidate, .by_group = TRUE) |>
  dplyr::slice(1) |>
  dplyr::ungroup() |>
  dplyr::transmute(
    row_id,
    name_trait  = candidate,
    match_trait = kind
  )
...
dat <- summary_df |>
  ...
  dplyr::filter(!is.na(raster_path), !is.na(name_trait)) |>
  dplyr::mutate(
...
if (isTRUE(DO_GOMPERTZ_BIRDS) && !is.null(models)) {
  ...
  combo_key[ii_bird] <- stringr::str_squish(dat$diet_combo[ii_bird])
...
keep_cols <- c(
  ...
  "sdm_method", "raster_dir", "raster_file", "raster_path",
...
```{r fig_area_curve_pterocles_personatus, fig.width=6.4, fig.height=3.3}
suppressPackageStartupMessages({
  library(readr)
  library(dplyr)
  library(tibble)
...
# --- Load the species table from disk (no dependency on earlier chunks) ---
species_csv <- if (file.exists("species_table.csv")) {
  "species_table.csv"
} else {
  file.path("Data", "Clean", "species_table.csv")
}
if (!file.exists(species_csv)) {
  stop(
    "Could not find species_table.csv.\n",
    "Looked for:\n",
    "  - species_table.csv\n",
...
dat_out <- readr::read_csv(species_csv, show_col_types = FALSE, progress = FALSE)

# --- Figure theme: aligned with the other main-text figures ---
...
if (nrow(sp_row) == 0) {
  stop(paste0("Species not found in species_table.csv: ", sp_name), call. = FALSE)
}
...
if (!is.finite(dens) || dens <= 0 || !is.finite(alpha) || alpha <= 0 ||
  ...
  stop(
    paste0(
      ...
      "Check density, alpha_", curve_use, ", beta_", curve_use, ", and min_pop_size in species_table.csv."
    ),
    call. = FALSE
  )
}

# Effective coefficient for the area-form curve: c = alpha * density^{-beta}
c_eff <- alpha * dens^(-beta)

# Broad log-spaced grid; final x-limits are chosen later to focus on the informative range
...
# dP/dA = P * c_eff * beta * delta^{-(beta+1)}
log_dP <- (-z) + log(c_eff) + log(beta) - (beta + 1) * log(delta_grid)
dP     <- exp(log_dP)

# Add the threshold point explicitly so both curves reach the A0 line
df <- tibble::tibble(
  area_km2 = c(A0, A),
  P        = c(0, P),
  dP       = c(0, dP)
...
# Scale derivative so its maximum is 1
s_deriv <- max(df$dP[is.finite(df$dP)], na.rm = TRUE)
df <- df %>%
  dplyr::mutate(dP_scaled = dP / s_deriv)

# --- Determine the most informative plotting window ---
band_cut <- 0.35
band_df  <- df %>% dplyr::filter(dP_scaled >= band_cut)
...
# Force the plotting range to include A0 and leave a small margin to its left
...
# --- Helpers for label placement on a log x-axis ---
...
# Move persistence label left and down; right-align so text extends leftward
...
# Keep marginal-sensitivity label low and away from dashed line
...
# A0 label placed immediately adjacent to the threshold line
x_lab_a0 <- A0 * 1.15
y_lab_a0 <- 0.1
...
p <- ggplot2::ggplot(plot_df, ggplot2::aes(x = area_km2)) +
  ggplot2::annotate(
    "rect",
    xmin = x_band_lo,
    xmax = x_band_hi,
