...
# ---------------------------------------
GOMPERTZ_CURVE <- "q025"  # must match columns alpha_<curve>, beta_<curve>
STOP_IF_MISSING_GOMPERTZ <- TRUE

# ---------------------------------------
# Curve-specific Outputs directory
# ---------------------------------------
OUTPUT_DIR <- file.path("Spatial", paste0("Outputs_", GOMPERTZ_CURVE))
...
csr_symmetrize_undirected <- function(row_ptr, col_idx) {
  ...
  key <- as.numeric(from2) + (as.numeric(to2) - 1) * as.numeric(N)
  o   <- order(key)
  key <- key[o]
  from2 <- from2[o]
  to2   <- to2[o]
...
```{r}
# fasterRaster is optional. If unavailable/unusable, we fall back to terra in fragmentation.
USE_FASTERRASTER <- TRUE

if (requireNamespace("fasterRaster", quietly = TRUE)) {
  tryCatch({
    suppressPackageStartupMessages(library(fasterRaster))
    fasterRaster::faster(grassDir = "C:/Program Files/GRASS GIS 8.4")
    USE_FASTERRASTER <- TRUE
    message("fasterRaster ready: will use fasterRaster::clump() for contiguity.")
  }, error = function(e) {
    USE_FASTERRASTER <<- FALSE
    message("fasterRaster present but not usable (", conditionMessage(e), "). ",
            "Falling back to terra for contiguity (terra::patches).")
  })
} else {
  message("fasterRaster not installed. Falling back to terra for contiguity (terra::patches).")
}
```
...
pkgs_needed <- c("data.table", "terra", "sf", "fastmatch", "Rfast")
# fasterRaster is optional now
...
apply_patch_fragmentation_updates <- function(
  ...
  .split_pu_apply_threshold <- function(conn_mod, patch_area_vec, sp, pu_old, thr_pu, next_pu_id) {
    ...
    ## ==========================================================
  ## Helper: clump/patch labeling with optional fasterRaster
  ## ==========================================================
  .clump_rook <- function(r_sub) {
    use_fast <- isTRUE(get0("USE_FASTERRASTER", ifnotfound = FALSE, inherits = TRUE)) &&
      requireNamespace("fasterRaster", quietly = TRUE)

    if (use_fast) {
      cl <- terra::rast(fasterRaster::clump(fasterRaster::fast(r_sub), diagonal = FALSE))
    } else {
      # terra fallback: rook connectivity (4 directions)
      # patches() labels connected areas of identical values, like clump().
      if ("patches" %in% getNamespaceExports("terra")) {
        cl <- terra::patches(r_sub, directions = 4)
      } else {
        # very old terra fallback (just in case)
        cl <- terra::clumps(r_sub, directions = 4)
      }
    }

    # Defensive: some backends may encode background as 0; standardize to NA
    v <- terra::values(cl, mat = FALSE)
    if (any(v == 0L, na.rm = TRUE)) {
      v[v == 0L] <- NA_integer_
      cl <- terra::setValues(cl, v)
    }
    cl
  }
...
resume_state <- readRDS(
  file.path(OUTPUT_DIR, sprintf("global_state_stage_%04d.rds", stage_n))
)
