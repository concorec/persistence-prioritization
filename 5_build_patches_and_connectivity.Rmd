...
# =============================================================================
paths <- list(
  # Canonical species table produced by 4_build_species_table.Rmd
...
assert(all(nzchar(sp$raster_path[selected])),
       "species_table.csv has empty raster_path in row(s) selected for processing.")

missing_rasters <- unique(sp$raster_path[selected & (!file.exists(sp$raster_path))])
assert(
  length(missing_rasters) == 0,
  paste0(
    "species_table.csv contains raster_path value(s) that do not exist on disk for selected row(s):\n",
    paste(head(missing_rasters, 20), collapse = "\n")
  )
)
...
# =============================================================================
# 7) fasterRaster + GRASS backend (required)
# =============================================================================
...
for (i in seq_len(nrow(species_df))) {
  ...
  log_step(
    sci,
    paste0(
      "RASTER | method=", species_df$sdm_method[i],
      " | file=", basename(species_df$raster_path[i])
    )
  )
  
  # ---- presence raster (already validated in chunk 1)
  pres_path <- species_df$raster_path[i]
...
```{r}
## ---- build_binary_patch_rasters_blockwise -----------------------------------
library(data.table)
library(terra)
library(stringr)

filtered_species_csv <- "Data/Clean/species_table_with_patches_only.csv"
patch_dir            <- "Data/Clean/Patches"
out_dir              <- "Data/Clean/Patches_binary_with_patches_only"

dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

# Optional: rebuild folder from scratch
unlink(list.files(out_dir, pattern = "\\.tif$", full.names = TRUE))

# Give terra a temp directory with plenty of space
terra_tmp <- file.path(tempdir(), "terra_tmp")
dir.create(terra_tmp, recursive = TRUE, showWarnings = FALSE)
terraOptions(memfrac = 0.2, tempdir = terra_tmp, progress = 1)

sp <- fread(filtered_species_csv)

species_id_from_scientific <- function(sci) {
  x <- str_squish(as.character(sci))
...
sp[, species_id := species_id_from_scientific(scientificName)]
sp[, patch_file := file.path(patch_dir, paste0(species_id, ".tif"))]
sp <- sp[file.exists(patch_file)]

for (i in seq_len(nrow(sp))) {
  in_file  <- sp$patch_file[i]
  out_file <- file.path(out_dir, paste0(sp$species_id[i], ".tif"))

  cat("Processing:", basename(in_file), "\n")

  r <- rast(in_file)

  # Output raster template
  out <- rast(r)

  # Start block-wise read/write
  readStart(r)
  out <- writeStart(
    out,
    filename = out_file,
  ...
  for (j in seq_len(bs$n)) {
    v <- readValues(r, row = bs$row[j], nrows = bs$nrows[j], mat = FALSE)

    # Patch rasters are patch IDs / NA
    # Convert any non-NA value to 1, keep NA as NA
    v[!is.na(v)] <- 1L
...
cat("Wrote binary patch rasters to:", out_dir, "\n")
cat("Files written:", nrow(sp), "\n")
```
